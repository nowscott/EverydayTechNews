...
- `SUBSCRIPTION_CONFIRMATION_SECRET`：确认与退订令牌签名密钥，建议使用至少 32 字节的随机值，并与 GitHub Actions 中的同名 secret 保持一致
...
- `Name`：标题字段
- `Email`：邮箱字段
- `状态` 或 `Status`：选择或状态字段，并包含 `待确认`、`正常`、`已退订`、`异常` 选项

新提交的邮箱先写入 `待确认`，确认邮件中的签名链接有效期为 24 小时，并在邮件中展示准确的北京时间失效时间。链接会打开独立的简洁确认页面，订阅者点击页面上的确认按钮后状态才更新为 `正常`，避免邮件安全扫描器误触发；同一令牌再次确认只会显示已使用。确认成功后会向订阅者另发一封成功邮件，并发送可选的站长通知。

每日邮件的退订链接由 Python 发送端按收件邮箱签名，有效期为 45 天。链接只打开退订确认页，用户主动点击确认后才更新为 `已退订`。`待确认` 邮箱可以重新提交以获取新链接；`已退订` 邮箱重新提交时会恢复为 `待确认` 并重新走确认流程；已经为 `正常` 的邮箱不会重复发送确认邮件，`异常` 邮箱不会自动恢复。
