...
新提交的邮箱先写入 `待确认`，确认邮件中的签名链接有效期为 24 小时。链接会打开独立的简洁确认页面，订阅者点击页面上的确认按钮后状态才更新为 `正常`，避免邮件安全扫描器误触发；同一令牌再次确认只会显示已使用。确认成功后会向订阅者另发一封成功邮件，并发送可选的站长通知。`待确认` 邮箱可以重新提交以获取新链接；`已退订` 邮箱重新提交时会恢复为 `待确认` 并重新走确认流程；已经为 `正常` 的邮箱不会重复发送确认邮件，`异常` 邮箱不会自动恢复。
